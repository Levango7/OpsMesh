...
//
// TODO(M15): 本文件与 pkg/cron/cron.go 存在双份实现，需手动同步。
// pkg/cron 为 services/task-svc 跨模块访问迁出的副本（go workspace 隔离）。
// 当前两份有故意差异（本实现正确处理 7=周日且边界校验更严格；
// pkg/cron 语法组合更丰富但 7=周日未规范化），系 A-1 阶段双轨对照设计。
