...
echo "[opsmesh] token written to $DATA_DIR/install.token (mode 0600)"
...
[Service]
ExecStart=$AGENT_BIN --control-addrs=$ADVERTISE --data-dir=$DATA_DIR
Restart=always
User=root
...
UNIT
  ...
  "$AGENT_BIN" --control-addrs="$ADVERTISE" --data-dir="$DATA_DIR" &
