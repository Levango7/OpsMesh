...
func (s *Server) scheduleCreate(w http.ResponseWriter, r *http.Request) {
	...
	if body.TaskID == "" {
		...
		paginate.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "taskID is required (pointing to a template task with Schedule set)"})
		return
	}
...
func (s *Server) handleScheduleRouting(w http.ResponseWriter, r *http.Request) {
	...
	if idAndRest == "" {
		paginate.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "schedule id required"})
		return
	}
...
func (s *Server) scheduleGet(w http.ResponseWriter, r *http.Request, id string) {
	...
	paginate.WriteJSON(w, http.StatusOK, e)
}
