...
// ============================================================
// K8s 管理（Phase 3）
// 包含：集群 CRUD、资源列表（namespace/pod/deployment/service/configmap/secret/node）、
//       pod 日志查看、pod 删除、deployment 扩缩容/重启。
// 设计要点：
//   1. 集群管理区域：表格列出所有集群，行内操作（测试连接 / 删除 / 进入资源管理）；
//   2. 资源管理区域：集群选择下拉 + namespace 输入 + 资源类型 tab + 资源列表表格；
//   3. 弹窗用 .dev-modal-overlay + .open class（与中间件部署对话框风格一致）；
//   4. 所有用户输入用 esc() 转义，避免 XSS；
//   5. 所有界面文字用 t() 翻译，支持中英文切换。
// ============================================================
