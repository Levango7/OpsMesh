...
//   - Keyword → 日志管道 |= "keyword"（LogQL 字面子串匹配，向后兼容，避免正则元字符改变语义）
...
// Keyword 用 |=（LogQL 字面子串匹配管道），向后兼容且安全：
// 用户传入的 keyword 作为字面量匹配，不会被当作正则解释，避免元字符（如 . * ( ) |）
// 改变匹配语义。若需正则能力，应由调用方显式选择并经独立 API 暴露。
func (s *LokiStore) buildLogQL(q Query) string {
	...
	if q.Keyword != "" {
		// |= 为 LogQL 字面子串匹配管道，向后兼容且安全：
		// keyword 原样作为字面量匹配，不会被当作正则解释，避免元字符改变匹配语义。
