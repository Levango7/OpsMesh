...
`, "task-"+a.AgentID+"-1", a.AgentID, a.TenantID, "shell", "uname -a", now)
	...
	// 审计留痕（等保三级：注册 100% 入审计轨迹；存储层统一产出，避免与 handler 重复）。
...
func (s *SQLStore) RetireStaleDevices(maxAge time.Duration) int {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	// leader fencing：非有效租约持有者跳过，防双主同时归档。
...
// RenewLeadership 选主：原子抢占或续租 leader_lease 单行（id=1）。
