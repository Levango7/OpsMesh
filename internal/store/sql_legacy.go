...
// initSchemaExtra 建 /111 新增的表（alert_rules/os_templates/middleware_templates/
// refresh_tokens）+ tasks 表审批字段列。由 sql.go initSchema 末尾调用。
//
// 拆分到独立文件避免 sql.go 被外部工具重置时丢失建表语句（工程债）。
//
// G5 / 建表逻辑双写治理：
//
...
func (s *SQLStore) initSchemaExtra(ctx context.Context) {
	// 任务审批：tasks 表增加审批字段列（向后兼容，老库无列时补列）。
	...
	// alert_rules 表 tenant_id 列兼容补丁（表本身由 003_legacy_tables.sql 创建）。
	s.alterColumnIfMissing(ctx, "alert_rules", "tenant_id", "VARCHAR(64)")
	// os_templates 表 tenant_id 列兼容补丁（表本身由 003_legacy_tables.sql 创建）。
	s.alterColumnIfMissing(ctx, "os_templates", "tenant_id", "VARCHAR(64)")
	// middleware_templates 表 tenant_id 列兼容补丁（表本身由 003_legacy_tables.sql 创建）。
	s.alterColumnIfMissing(ctx, "middleware_templates", "tenant_id", "VARCHAR(64)")
	// refresh_tokens 表 tenant_id 列兼容补丁与索引补丁
