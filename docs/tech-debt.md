...
| ID | 问题 | 现状 / 影响 / 下一步 |
|---|---|---|
| TD-60 | **七域双份实现收敛**（约 27,000 行重复） | **2026-09-05 产品决策：选项 A——微服务化为正式方向**。同日深度侦察（两路 agent 全仓盘点）后拆为两阶段：**阶段 1（已完成）**：轻四域（gpu/runbook/incident/autoscaler）经侦察确认 controlplane 侧已纯 service_proxy 代理化（零本地实现）——迁移目标状态实际已就位，遗留缺口是前端契约对齐层（4 个必 404+2 个 405+3 处字段断裂），已全部修复并补 22 个 handler 防回归测试（e2e757c+67a1050，CI 全绿）。**阶段 2（已立项进行中，核心三域 task/device/auth 迁移）——A-1 第一批 task-svc 双轨对照补齐已完成（a4d819d+c8bc8ca，scheduler 3 循环+ValidateCommand 移植+pkg/cron 迁出）**：侦察结论=现阶段不可薄客户端化——task-svc 缺四大任务必达机制（DAG 释放/定时派生 scheduleLoop/租约回收 reclaim/leader 选主，缺失会让失联任务永久卡死+定时全停）；device-svc 的 StartDiscovery 是硬编码 stub（写死 3/254）+自动纳管链路（install token 签发/消费、install.sh/agent 分发、SSH 推送、后台循环）全缺；auth-svc 缺 HttpOnly Cookie/防爆破 loginGuard/注册审批，而 controlplane requireProd 被 121 处调用依赖本地 JWT 验签（全量远程化=每请求加一跳 RPC+auth 故障全站 401）。**最大耦合点**：agent gRPC 通道（GrpcServerImpl，PullTasks/ReportResult/Heartbeat/Register 四条直写 store）是 agent 管控数据面——agent 只说 opsmesh.v1.Registration 协议不认微服务协议，该通道保留 controlplane 实现（阶段 2 需给其加"内部转调微服务"的适配层决策）。**阶段 2 前置补齐清单**：device-svc 接真实 Sweep/install token/bootstrap 资产分发/PushAndExec/autoProvisionLoop/心跳归档/metrics；auth-svc 补 loginGuard/注册审批/validateStrongPassword/DeviceFP/Redis SessionStore + 与 controlplane 统一 JWT claims 契约（共享密钥本地验签模式）。**阶段 2 收尾（A-2 待解锁）**：A-1 补齐完成 → **双轨跑对比**→ 一致性产出报告 → 一致的切流 50/50 → 全绿稳定 1 周后 100% 切流 → 下掉 controlplane 实现——您选中的稳妥节奏：不激进、可回滚、不出问题再切换。 **A-2 已完成（4a85fef, 2026-09-17）**：task-svc 恢复完整生产路径（arm64 多架构 Dockerfile + EventBus/AuditSink/SSEBridge 事件总线/审计/SSE 桥接 + K8s Lease LeaderElector 选主）；device-svc 全量补齐（agent 二进制按平台/架构分发 + AutoProvisionLoop 指数退避 + 设备 metrics）；auth-svc 安全补齐（JWT device_fp claim + 12+特殊字符密码策略+黑名单 + DeviceFP 采集 UA+IP+TLS→hash + Redis SessionStore + loginGuard Redis 后端降级内存）。全部接口注入模式，Redis 不可用时降级运行。**双轨 API 一致性对比报告已完成（b7d28b5, 2026-09-17）**：task 60% / device 60% / auth 86.7%，详见 docs/td60-consistency-report.md。**P0 阻塞项补齐已完成（1258289, 2026-09-17）**：task-svc 新增 HTTP 业务网关（12 个 REST 端点，693 行）；device-svc 补 provision/metrics/聚合响应 + MetricsStore 接口注入；auth-svc 补 PUT users/roles + requirePermission。三域 build/test/vet 全绿。**双轨本地 Docker 验证已完成（1d74d34, 2026-09-17）**：6 容器全部健康（controlplane+task-svc+device-svc+auth-svc+mysql+redis），P0 端点全部可达（task-svc 12 REST 端点 200 / device-svc provision+metrics 404 合理 / auth-svc PUT users/roles 401 需认证），修复 device-svc 路由冲突 panic。**下一步：50/50 切流 → 全绿稳定 1 周后 100% 切流 → 下掉 controlplane 实现** **本轮阻塞修复+双轨E2E验证（d449718, 2026-09-17）**：task-svc SQL错误贯穿修复+batch_id启动迁移（幂等并发安全）+nullTime修复；auth-svc Redis显式注入+管理路由鉴权（13路由x3凭证态）+user:approve权限对齐controlplane+改密参数校验。双轨栈E2E全通过：task创建到列表可见+auth Redis连接redis:6379+首登改密到管理端点200/无token 401。仍未完成：无（本轮已修复调度并发风险+列表格式对齐，e05c2fb）。 |
...
| TD-65 | ~~**微服务 MySQL store 未接线**~~ | ✅ 2026-08-30 已修（9/12 接线）：alert/auth/config/deploy/device/incident/plugin/portal/task 经 `<NAME>_SVC_STORE_TYPE=sql` + DSN 启用 MySQL；缺字段服务补 config，mysql.go 补编译期断言；**附带安全修复**：config-svc 硬编码 `deriveKey("default-key")` 改为 cfg.EncryptionKey（跨后端加密一致）。跳过项与原因：runbook（无实现）/ gpu（manager 不消费 store，需重构）/ workflow（接口签名不兼容，需适配层）/ log（main 已有 memory/sql/loki/es 四后端分支，不应换接）。留此行供集成测试补齐后删除。**2026-09-25 修订**：原描述的「失败回退 memory」已被证伪为生产陷阱（见 TD-68）——「接线成功」并不等于「数据真的落库」，本轮以真机建表实证收口 |
| TD-69 | **迁移安全：并发启动踩踏 / 无回滚 / 半应用不可续（P0-5）** | ✅ 2026-09-25 已修：①启动期迁移在单 `*sql.Conn` 上取 `GET_LOCK('opsmesh_mig_<db>',60)` 咨询锁，多副本并发启动串行化；②`schema_migrations` 记录 SHA-256 checksum，既有迁移文件被改动即启动 fatal；③库内版本 > 二进制已知最大版本时拒绝启动；④半途失败可重放续跑（`1050/1060/1061/1091` 仅在回查 `information_schema` 确认后容忍），`NewSQLStore` 迁移失败 fail-fast；⑤补齐 18 个 `.down.sql`（**永不自动执行**）+ `docs/operations.md` §6.3.2 回滚对照表与 4 步手工流程。集成验证：真实 MySQL 8.0.46 上 13 个迁移集成测试全绿 |
| TD-70 | **跨租户越权：可让他人 agent 以 root 执行任意脚本（P0-6）** | ✅ 2026-09-25 已修：①`users.tenant_id` 迁移（018）+ 历史行回填 `default` + JWT `tenant_id` 声明；②用户创建/更新按调用方租户收敛 + 租户 ID 字符集校验（`^[A-Za-z0-9_.-]{1,64}$`）；③新增 `internal/controlplane/tenant_guard.go` 统一校验入口，接入 4 条「用户指定 agentID 下发任务」路径（批量先整体校验再落库）；④领取侧在 SQL 层加租户门 `AND (tenant_id IS NULL OR tenant_id='' OR tenant_id=?)`，兼容存量空租户任务；`Store` 接口刻意不变以免波及 35 个子接口。回归断言见 `deploy/scripts/verify-runtime.sh` §11 与 3 个新增测试文件 |
| TD-68 | **微服务持久化静默降级 + DSN 二次追加（P0-7）** | ✅ 2026-09-25 已修：①10 个微服务 `ensureParseTime` 在 DSN 已含 `parseTime=true` 时二次追加，产生 `invalid bool value: true?parseTime=true` → `sql.Open` 必失败 → 静默回退 memory（生产数据重启即丢、`/health` 仍绿）。改为幂等归一化并补 `dsn_test.go`（10 模块）；②`StoreType=sql` + DSN 显式配置下的初始化失败由 `log.Printf` 改为 `log.Fatalf`（8 个 main.go，对齐 task-svc 与 controlplane `--production` 的 fail-fast 先例），杜绝「声明持久化却跑在内存里」。回归断言：`verify` 脚本 §8 校验 `opsmesh_alert.alerts` / `opsmesh_config.config_entries` 可查 |
