...
| 前端已接入子域数 | 27 | 对应 `web/enterprise/src/api/*.js` 39 个模块覆盖的功能域（部分模块共享子域，如 auth.js 覆盖 auth+users+roles+permissions） |
| 未接入子域数 | 13 | 后端有路由注册但前端无对应 API 模块、无 Pinia store、无视图组件、无路由注册 |
...
### 2.6 【P1】多集群联邦部署（deploys-federation）
