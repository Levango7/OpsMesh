...
> 版本：v1.0 · 编制日期：2026-08-17 · 适用基线：OpsMesh MVP + M2 演进
>
...
| 编号 | 原则 | 含义 |
|---|---|---|
| | 单二进制双模式 | 控制面与 Agent 共用同一二进制，`--mode=controlplane\|agent` 切换，降低交付与升级成本 |
| | 渐进式高可用 | 从单机 memory store 起步，按规模渐进切换 mysql + 多副本 + 联邦，避免过度设计 |
...
- **告警通知**：短信+电话+邮件，短信+邮件，P3 邮件，P4 仅记录
