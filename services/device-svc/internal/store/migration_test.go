...
// TestSplitStatements_DeviceSchema 验证嵌入的 schema.sql 能被完整拆成可执行语句。
// 建表此前从未执行过（无任何代码读取 schema.sql，mysql.go 内也无 CREATE TABLE），
// 故这里既校验数量也校验表名与顺序，防止将来改动 schema.sql 后迁移静默漏建表。
func TestSplitStatements_DeviceSchema(t *testing.T) {
	stmts := splitStatements(deviceSchema)
	if len(stmts) != 5 {
		t.Fatalf("语句数 = %d, want 5（devices/agents/ci_items/ci_relations/discovery_jobs）", len(stmts))
	}
	want := []string{"devices", "agents", "ci_items", "ci_relations", "discovery_jobs"}
